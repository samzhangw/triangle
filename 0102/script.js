document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyMoveToBoard(dotA, dotB, player) {
        ...
        const lineSum = valA + valB;
        if (player === 1) p1PointSum += lineSum;
        else if (player === 2) p2PointSum += lineSum;
        updatePointSumUI();
        addMoveToHistoryUI(player, valA, valB, lineSum);
    ...
    // 添加一筆紀錄到列表
    function addMoveToHistoryUI(player, valA, valB, sum) {
        ...
        li.innerHTML = `
            ...
            <span>
                ${valA} + ${valB} = <span class="sum-badge">${sum}</span>
            </span>
        `;
