...
const QUIESCENCE_MAX_DEPTH = 3; // (速度平衡版)
...
function evaluateBoard(currentLines, currentTriangles) {
    ...
    if (isScoreAndGoAgain) {
        // 策略: 積極進攻。
        // 製造威脅 (p2Threats) 是 *好事* (+25)
        // 對手製造威脅 (p1Threats) 是 *壞事* (-25)
        p1ThreatWeight = -25; 
        p2ThreatWeight = 25;
        p1DoubleWeight = -75;
        p2DoubleWeight = 75;
        ...
        // 策略: 龜縮防守。
        // 製造威脅 (p2Threats) 是 *壞事* (-25)
        // 對手製造威脅 (p1Threats) 是 *好事* (+25)
        p1ThreatWeight = 25;
        p2ThreatWeight = -25;
        p1DoubleWeight = 75;
        p2DoubleWeight = -75;
    ...
    return (p2Score * 150 - p1Score * 150) +
           (p1Threats * p1ThreatWeight + p2Threats * p2ThreatWeight) +
           (p1DoubleSetups * p1DoubleWeight + p2DoubleSetups * p2DoubleWeight);
}
...
function quiescenceSearch(currentLines, currentTriangles, depth, isMaximizingPlayer, alpha, beta) {
    ...
    if (isMaximizingPlayer) { // P2 (Max)
        ...
        // (**** 新功能 ****) 只有在「得分後再走一步」規則開啟時，才繼續搜尋
        // 否則，靜態搜尋沒有意義（因為得分就換人了）
        if (isScoreAndGoAgain) {
...
function minimax(currentLines, currentTriangles, depth, isMaximizingPlayer, alpha, beta) {
    ...
    if (isMaximizingPlayer) { // P2 (AI) 的回合 (Maximizer)
        ...
        for (const move of allMoves) {
            ...
            const futureValue = minimax(sim.newLines, sim.newTriangles, depth - 1, isStillMaximizing ? true : false, alpha, beta);
            const totalValue = immediateScore + futureValue; 
        ...
        for (const move of allMoves) {
            ...
            const futureValue = minimax(sim.newLines, sim.newTriangles, depth - 1, isStillMinimizing ? false : true, alpha, beta); 
            const totalValue = -immediateScore + futureValue; 
