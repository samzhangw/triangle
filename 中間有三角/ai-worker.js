/**
 * ============================================
 * AI Web Worker (ai-worker.js) - 支援 3D 立體多面體與科展六邊形
 * ============================================
 */
...
let is3DMode = false; 
let isHoneycombMode = false; // 👈 專門辨識科展棋盤，以解除思考深度限制
...
self.onmessage = (e) => {
    ...
    if (data.command === 'start') {
        ...
        isHoneycombMode = data.gameState.isHoneycombMode === true; // 👈 讀取科展模式
        ...
        isHoneycombMode = data.gameConfig.isHoneycombMode === true;
        ...
        is3DMode = data.gameState.is3DMode === true;
        isHoneycombMode = data.gameState.isHoneycombMode === true;
...
function findIntermediateDots(dotA, dotB) {
    // 3D 或 科展圖形直接當作沒有中繼點的圖論結構
    if (is3DMode || isHoneycombMode) return [dotA, dotB]; 
...
function isValidPreviewLine(dotA, dotB, currentLines) {
    ...
    // 如果是 3D 或 科展圖形，跳過平面幾何判定，只要字典裡有這條線且還沒畫過就合法
...
function findAllValidMoves(currentLines) {
    ...
    // 🚀 效能大躍進！如果是科展圖形或 3D 模式，跳過 N^2 的點對點掃描
    // 直接掃描現有的線條字典，O(1) 找出所有還沒畫過的邊
    if (is3DMode || isHoneycombMode) {
        for (const id in currentLines) {
            if (!currentLines[id].drawn) {
                moves.push({
                    dot1: currentLines[id].p1,
                    dot2: currentLines[id].p2,
                    segmentIds: [id]
                });
            }
        }
...
function getAIDepth() { 
    // 🧠 核心修復：解除封印！
    // 如果是科展圖形，維持高智商深度為 6（配合閃電搜尋，效能可負荷）
    if (isHoneycombMode) return 6; 
    
    // 3D 模式因為維度與邊數過大，預設降階至 4 以免卡死
