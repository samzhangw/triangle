document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeAIWorker() {
        ...
        aiWorker.onmessage = (e) => {
            ...
            if (data.type === 'log') {
                ...
                // **** 新增：處理訓練結果 ****
                handleTrainingGenerationComplete(data.population);
            }
        };
        aiWorker.onerror = (e) => {
    ...
    function initGame() {
        ...
        triangles = [];
        totalTriangles = 0;
        // 這裡的三角形產生邏輯需要非常小心，避免遺漏
        // 為簡化代碼，使用一種通用的偵測方式：
        // 遍歷所有點，找出能構成三角形的三個點 (距離為1)
    ...
    // 輔助函式：判斷三點是否構成有效的單位三角形
    function isValidTriangle(d1, d2, d3) {
        // 檢查三個點是否兩兩相連且距離為1單位 (簡化判斷: 檢查是否有這三條線)
    ...
    function handleTrainingGenerationComplete(populationWithFitness) {
        if (!isTraining) return;
