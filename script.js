document.addEventListener('DOMContentLoaded', () => {
    ...
    function initGame() {
        ...
        const isP1AI = (gameMode === 2); // (gameMode 在批次模式下會被設為 2)
    ...
    function endGame() {
        ...
        if (isBatchRunning) {
            ...
            if (batchGamesCompleted < batchTotalGames) {
                // 還有下一場
                setTimeout(initGame, 10); // 立即開始下一場 (用短暫停避免瀏覽器卡死)
            } else {
                // 批次執行完畢
                isBatchRunning = false;
                toggleUIControls(true); // 解鎖 UI
                if (batchStatusMessage) {
                    batchStatusMessage.textContent = `批次完成！已匯出 ${batchTotalGames} 場紀錄。`;
                }
                exportBatchLog(); // 自動匯出批次紀錄
            }
    ...
    function triggerAIMove() {
        ...
        isAIThinking = true;
        if (aiThinkingMessage && !isBatchRunning) aiThinkingMessage.classList.remove('hidden'); // (新) 批次模式下不顯示
        if (aiLogContainer && !isBatchRunning) aiLogContainer.classList.remove('hidden'); // (新) 批次模式下不顯示
    ...
    // 綁定 PNG 匯出按鈕
    if (exportPNGButton) exportPNGButton.addEventListener('click', exportCanvasAsPNG);
    if (exportPNGButtonModal) exportPNGButtonModal.addEventListener('click', exportCanvasAsPNG);
