document.addEventListener('DOMContentLoaded', () => {
    ...
    // [新增] 分析與模擬的建議點變數
    let analysisDot1 = null; 
    let analysisDot2 = null; 
    ...
    function initializeAIWorker() {
        ...
        aiWorker.onmessage = (e) => {
            ...
            // ==========================================
                ...
                isAIThinking = false;
                isAnalyzing = false;
                handleScoreSimulationResult(data);

            // ==========================================
            // [新增] 最高分生成結果 (Generator)
            // ==========================================
            } else if (data.type === 'high_score_result') { 
                handleHighScoreResult(data);

            } else if (data.type === 'result') {
                isAIThinking = false;
                
                // [新增] 判斷是否為分析模式 (Minimax)
                if (isAnalyzing) {
                    isAnalyzing = false;
                    handleAnalysisResult(data.bestMove);
                } else {
    ...
    function drawCanvas() {
        ...
        // [新增] 繪製分析建議線 (紫色虛線 - 規則分析)
        if (analysisDot1 && analysisDot2) {
            ctx.beginPath();
            ctx.moveTo(analysisDot1.x, analysisDot1.y);
            ctx.lineTo(analysisDot2.x, analysisDot2.y);
            ...
            [analysisDot1, analysisDot2].forEach(d => {
                ctx.beginPath();
                ctx.arc(d.x, d.y, DOT_RADIUS + 5, 0, 2 * Math.PI);
        ...
        // [新增] 繪製模擬建議線 (金色虛線 - 最高分模擬)
        if (simDot1 && simDot2) {
            ctx.beginPath();
            ctx.moveTo(simDot1.x, simDot1.y);
            ctx.lineTo(simDot2.x, simDot2.y);
            ...
            // 繪製起終點光暈
            [simDot1, simDot2].forEach(d => {
                ctx.beginPath();
                ctx.arc(d.x, d.y, DOT_RADIUS + 6, 0, 2 * Math.PI);
    ...
    if (analyzeMoveBtn) {
        analyzeMoveBtn.addEventListener('click', startAnalysis);
    }

    function startAnalysis() {
        if (isAIThinking || isBatchRunning || isAnalyzing) return;
        ...
        isAnalyzing = true;
        isAIThinking = true; // 鎖定 UI 避免衝突
        
        if (aiThinkingMessage) {
            aiThinkingMessage.textContent = "🔮 AI 正在分析無連鎖最佳策略...";
            aiThinkingMessage.classList.remove('hidden');
        }

        // 建構分析用的 Payload，強制 isScoreAndGoAgain = false
        const payload = {
            command: 'start',
            aiType: 'minimax', // 強制使用 Minimax 進行最佳解分析
            weights: null, 
            gameState: {
                dots: dots,
                lines: lines,
                ...
                totalTriangles: totalTriangles,
                requiredLineLength: REQUIRED_LINE_LENGTH,
                isScoreAndGoAgain: false, // ★ 關鍵：強制關閉「得分再走」
                allowShorterLines: isAllowShorterLines 
            }
        };
    ...
    function handleAnalysisResult(bestMove) {
        if (aiThinkingMessage) aiThinkingMessage.classList.add('hidden');
        if (aiThinkingMessage) aiThinkingMessage.textContent = "電腦思考中...";

        if (bestMove && bestMove.dot1 && bestMove.dot2) {
            const d1 = dots[bestMove.dot1.r][bestMove.dot1.c];
            const d2 = dots[bestMove.dot2.r][bestMove.dot2.c];
            
            analysisDot1 = d1;
            analysisDot2 = d2;
            drawCanvas();
            
            const coordStr = `(${d1.r},${d1.c}) ↔ (${d2.r},${d2.c})`;
            logAI(`★ 分析結果: 建議走法 ${coordStr}`);
            alert(`🔮 分析完成！\n\n在「沒有得分後再走一步」的規則下，\nAI 建議的最佳走法是：${coordStr}\n\n(已在棋盤上用紫色虛線標示)`);
        } else {
            alert("無法分析出有效走法 (可能已無棋可走)。");
        }
    }

    // ============================================
    // [新增] 模擬最高分策略 (Monte Carlo)
    // ============================================
    const simulateScoreBtn = document.getElementById('simulate-score-btn');
    if (simulateScoreBtn) {
        simulateScoreBtn.addEventListener('click', startScoreSimulation);
    }
    ...
    function startScoreSimulation() {
        ...
        simDot1 = null;
        simDot2 = null;
        analysisDot1 = null; // 同時清除紫線
        analysisDot2 = null;
        drawCanvas();

        isAnalyzing = true; // 重用 analyzing 狀態鎖定
        isAIThinking = true; 
        
        if (aiThinkingMessage) {
            aiThinkingMessage.textContent = "🎰 正在模擬 500+ 場對局以計算最高分期望值...";
            aiThinkingMessage.classList.remove('hidden');
        }

        const payload = {
            command: 'analyze_score_simulation', // 新指令
            gameState: {
                dots: dots,
                lines: lines,
                triangles: triangles,
                ...
                totalTriangles: totalTriangles,
                requiredLineLength: REQUIRED_LINE_LENGTH,
                isScoreAndGoAgain: isScoreAndGoAgain, // 使用當前規則
                allowShorterLines: isAllowShorterLines 
            }
        };

        // 目前模擬功能僅支援本機 Worker (速度較快且無需 API 修改)
        logAI(`--- [本機] 啟動蒙地卡羅分數模擬... ---`);
        if (aiWorker) aiWorker.postMessage(payload);
    }

    function handleScoreSimulationResult(result) {
        if (aiThinkingMessage) aiThinkingMessage.classList.add('hidden');
        if (aiThinkingMessage) aiThinkingMessage.textContent = "電腦思考中...";

        if (result.bestMove && result.bestMove.dot1 && result.bestMove.dot2) {
            const d1 = dots[result.bestMove.dot1.r][result.bestMove.dot1.c];
            const d2 = dots[result.bestMove.dot2.r][result.bestMove.dot2.c];
            ...
            const coordStr = `(${d1.r},${d1.c}) ↔ (${d2.r},${d2.c})`;
            const avgScore = result.avgScore.toFixed(2);
            logAI(`★ 模擬結果: 建議走法 ${coordStr}, 預期得分 ${avgScore}`);
            
            alert(`🎰 模擬完成！\n\n經過大量隨機對局模擬，\nAI 預測這一步能讓您最終獲得最高分 (平均約 ${avgScore} 分)。\n\n(已在棋盤上用金色虛線標示)`);
        } else {
            alert("模擬無法找出有效走法 (可能已無棋可走)。");
        }
    }

    // ============================================
    // [新增] 產生最高分佈局 (Generator)
    // ============================================
    const generateHighScoreBtn = document.getElementById('generate-high-score-btn');
    if (generateHighScoreBtn) {
        generateHighScoreBtn.addEventListener('click', startHighScoreGeneration);
    }

    function startHighScoreGeneration() {
        if (isAIThinking || isBatchRunning) return;
        
        if (!confirm("這將會重置當前棋盤，並透過 AI 自我對戰產生一局「最高分佈局」範例。\n\n確定要繼續嗎？")) return;
        ...
        if (aiThinkingMessage) {
            aiThinkingMessage.textContent = "🏆 正在模擬 1000 場強弱對決，尋找最佳佈局...";
            aiThinkingMessage.classList.remove('hidden');
        }

        const payload = {
            command: 'generate_high_score',
            gameConfig: {
                dots: dots,
                lines: lines, // 此時 lines 已經是空的 (因 initGame)
                triangles: triangles,
                player: currentPlayer,
                totalTriangles: totalTriangles,
                requiredLineLength: REQUIRED_LINE_LENGTH,
    ...
    function handleHighScoreResult(result) {
        ...
        for (const id in result.finalLines) {
            if (lines[id]) {
                ...
                lines[id].player = result.finalLines[id].player;
                lines[id].sharedBy = result.finalLines[id].sharedBy;
            }
        }
